...
(function() {
  $(document).on("page:change", function() {
    return $('#submit-form').on('submit', function(evt) {
      var accessToken, handle, profile_request_url;
      evt.preventDefault();
      $('#search-images-section').toggle();
      handle = $('#handle-search-text');
      accessToken = $('#hidden-access').val();
      profile_request_url = "https://api.instagram.com/v1/users/search?q=" + handle.val() + "&access_token=" + accessToken;
      ...
      return $.ajax({
        ...
        success: function(data) {
          var current_handle_id, recent_media_request_url;
          console.log(data);
          ...
          return $.ajax({
            ...
            cache: false,
            success: function(data) {
              var $img, $linkParent, currentLink, i, n, product_pic, results, s;
              console.log(data);
              results = [];
              for (n = i = 0; i < 6; n = ++i) {
                s = "" + n;
                ...
                results.push($linkParent.attr('href', currentLink + "?url=" + encodeURIComponent(product_pic) + "&handle=" + handle));
              }
              return results;
            }
          });
